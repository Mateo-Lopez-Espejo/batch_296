...
def load_baphy_file(filepath):
    ...
    None. 'meta' contains:
        {'stimf':stimulus frequency,'respf':response frequency,'iso':isolation,
             ...
             poststim silence}
    """
    dat1 = dict.fromkeys(['stim', 'resp', 'pup'])
    meta = dict.fromkeys(['stimf', 'respf', 'iso', 'prestim', 'duration', 'poststim',
                          'tags', 'tagidx', 'ff'])
    matdata = si.loadmat(filepath, chars_as_strings=True)
    m = matdata['data'][0][0]
    dat1['resp'] = m['resp_raster']
    dat1['stim'] = m['stim']
    dat1['stimf'] = m['stimfs'][0][0]
    dat1['respf'] = m['respfs'][0][0]
    dat1['iso'] = m['isolation'][0][0]
    dat1['tags'] = np.concatenate(m['tags'][0]['tags'][0][0], axis=0)
    try:
        dat1['tagidx'] = m['tags'][0]['tagidx'][0][0]
        dat1['ff'] = m['tags'][0]['ff'][0][0]
    except:
        pass
    dat1['prestim'] = m['tags'][0]['PreStimSilence'][0][0][0]
    dat1['poststim'] = m['tags'][0]['PostStimSilence'][0][0][0]
    dat1['duration'] = m['tags'][0]['Duration'][0][0][0]
    try:
        dat1['pup'] = m['pupil']
    except:
        dat1['pup'] = None
